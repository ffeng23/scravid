...
knn<-readRDS(file=here("Output","knn.error.models_3rd_cd38low.Rds"))
...
sort(varinfo$arv, decreasing = TRUE)[1:30]
#
#varinfo <- pagoda.subtract.aspect(varinfo, colSums(cd[, rownames(knn)]>0))
#saveRDS(varinfo, file="varinfo_cd38low_3rd.Rds")
#####end of running section
varinfo<-readRDS(file=here("Output","varinfo_cd38low_3rd.Rds"))
...
df$GO_name<-get_names(df$name)$go_name
df$GO_root_name<-get_names(df$name)$root_node
...
##end of running section
load(file=here("Output","Output_cd38low_3rd.RData" ))
...
#(n.genes = 50 )
# as.list(go.env) to the list of genes.