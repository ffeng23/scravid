---
title: "GSEA using fgsea and msigdbr for apoptosis/BAFF/TACI related pathways"
author: "Feng, Feng"
date: "`r Sys.Date()`"
output:
...
In this exercise, we run GSEA for apoptosis/BAFF/TACI-related pathways using
...
We first extract all the pathways in msigdb database, and then search for related ones.
...
Now looking for specific pathways related to the target genes we are interested. First, let's look at the apoptosis pathways.
...
#pattern="BAFF|BAFFR|TNFSF13B",
ignore.case=T)

print(unique(pathway_names[nfkb_index]))


```

Second one is the BAFF related pathways.
```{r BAFF}
#First apoptosis
pathway_names<-m_df$gs_name
nfkb_index<-grep(x=pathway_names, 
...
Third one is the TACI pathways.
```{r TACI}
#TACI
pathway_names<-m_df$gs_name
nfkb_index<-grep(x=pathway_names, 
#pattern="apoptosis|apoptotic",
pattern="TACI|TNFRSF13B",
ignore.case=T)
...
write_tsv(x=as.data.frame(fgseaRes),
	file=here("Output","fGSEA_results_cvid_hc_apoptosis_all.tsv"))
```
...
plotEnrichment(fgsea_sets[["HALLMARK_APOPTOSIS"]],
               ranks) + labs(title="HALLMARK_APOPTOSIS")
print(fgseaResTidy, n=100)
```

## Plot heatmap of example hallmark apoptosis pathways
```{r heatmap, fig.width=6, fig.height=18}
gene_names<-fgsea_sets[["HALLMARK_APOPTOSIS"]]
...
Next we want to see the results for BAFF.
```{r BAFF_run}
m_df<- msigdbr(species = "Homo sapiens", category = NULL, 
    subcategory=NULL)
...
plotEnrichment(fgsea_sets[["GSE10422_WT_VS_BAFF_TRANSGENIC_LN_BCELL_UP"]],
               ranks) + labs(title="GSE10422_WT_VS_BAFF_TRANSGENIC_LN_BCELL_UP")
print(fgseaResTidy, n=100)  
```

Run heatmap to see gene-specific information.

```{r heatmap_BAFF, fig.width=6, fig.height=18}

gene_names<-fgsea_sets[["GSE10422_WT_VS_BAFF_TRANSGENIC_LN_BCELL_UP"]]
gene_dt<-GetAssayData(cvid.combined, slot = "data")
...
Now it is to see for TACI the analysis.
...
plotEnrichment(fgsea_sets[["MOREAUX_B_LYMPHOCYTE_MATURATION_BY_TACI_DN"]],
               ranks) + labs(title="MOREAUX_B_LYMPHOCYTE_MATURATION_BY_TACI_DN")
print(fgseaResTidy, n=100)  
```

Run heatmap to see gene-specific information.

```{r heatmap_TACI, fig.width=6, fig.height=18}

gene_names<-fgsea_sets[["MOREAUX_B_LYMPHOCYTE_MATURATION_BY_TACI_DN"]]
gene_dt<-GetAssayData(cvid.combined, slot = "data")
...
#library(pheatmap)
pheatmap(mat=gs_dts[1:83,],scale="row", show_rownames= T,
  show_colnames=T, labels_row=rownames(gs_dt),
  cluster_rows=T, cluster_cols=F, border_color="grey")
