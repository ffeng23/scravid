---
title: "GSEA using fgsea and msigdbr for PI3K related pathways"
author: "Feng, Feng"
date: "`r Sys.Date()`"
output:
...
In this exercise, we run GSEA for PI3K-related pathways using
...
pi3k_index<-grep(x=pathway_names, 
pattern="PI3K",
#pattern="BAFF|BAFFR|TNFSF13B",
ignore.case=T)

print(unique(pathway_names[pi3k_index]))
...
write_tsv(x=as.data.frame(fgseaRes),
  file=here("Output","fGSEA_results_cvid_hc_PI3K_all.tsv"))
```

We now  explore the data by plotting top ones and also show 
enrichment score for a specific gene set, e.g. hallmark PI3K.
...
head(n= 20), aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill= NES < 3.0)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="PI3K pathways NES from GSEA") + 
  theme_minimal()


plotEnrichment(fgsea_sets[["HALLMARK_PI3K_AKT_MTOR_SIGNALING"]],
               ranks) + labs(title="HALLMARK_PI3K")
print(fgseaResTidy, n=100)
```

## Plot heatmap of example hallmark PI3K pathways
```{r heatmap, fig.width=6, fig.height=18}
gene_names<-fgsea_sets[["HALLMARK_PI3K_AKT_MTOR_SIGNALING"]]
...
#library(pheatmap)
pheatmap(mat=gs_dts[1:109,],scale="row", show_rownames= T,
